...
- 当前主线：主线规划 21 期。已完成第 1 期 `ReAct`、第 2 期 `Plan-and-Execute + DAG`、第 3 期 `Memory + 上下文工程`、第 4 期 `RAG 检索 + 代码库理解`、第 5 期 `Multi-Agent 协作 + 角色分工`、第 6 期 `HITL 人工审批 + 危险操作拦截`（含 HITL 增强：路径围栏 / 命令快速拒绝 / 操作审计）、第 7 期 `异步执行 + 并行工具调用`、第 8 期 `多模型适配 + 运行时切换`、第 9 期 `联网能力 + Web 工具`、第 10 期 `MCP 协议核心（stdio + Streamable HTTP）`、第 11 期 `MCP 高级能力首批（resources 双轨 + prompts 查看 + 被动通知）`、第 12 期 `长上下文工程`、第 13 期 `Chrome DevTools MCP`、第 14 期 `CDP 会话复用 + 登录态访问`、第 15 期 `Skill 系统`、第 16 期 `TUI 产品化`、第 16.1 期 `形态修正：抽出 Renderer 接口 + 三个实现（inline / lanterna / plain），默认切换为 inline 流式 TUI（Claude Code 风格）`、第 17 期 `LSP 诊断注入` MVP、第 18 期 `Git Side-History 快照与回滚` MVP、第 19 期 `Prompt 分层架构` MVP、第 20 期 `异步后台任务 + Runtime API` MVP、第 21 期 `图片复制粘贴输入` MVP
- 下一步：OAuth / sampling / recovery 作为后续 MCP 增强，不属于当前第 16 期已交付范围
- 当前用户可感知版本：CLI Banner 显示 `v16.1.0`
- 当前 Maven 产物版本：`pom.xml` 仍是 `1.0-SNAPSHOT`
- 结论：如果你看到运行界面是 `v16.1.0`，但 Jar 名仍是 `paicli-1.0-SNAPSHOT.jar`，这是当前仓库的真实状态，不是你看错

## 运行前提

- Java 17+
- Maven
- 可用的 `GLM_API_KEY`、`DEEPSEEK_API_KEY`、`STEP_API_KEY` 或 `KIMI_API_KEY`（兼容 `MOONSHOT_API_KEY`）

API Key 当前读取顺序以代码为准：

1. `~/.paicli/config.json` 中对应 provider 的 `apiKey`
2. 环境变量：`GLM_API_KEY` / `DEEPSEEK_API_KEY` / `STEP_API_KEY` / `KIMI_API_KEY`（Kimi 兼容官方 `MOONSHOT_API_KEY`）
3. 仓库当前目录下的 `.env`
4. 用户主目录下的 `.env`
...
Side-Git 快照默认持久化位置（以代码实际行为为准）：

1. 系统属性：`-Dpaicli.snapshot.dir=/path/to/snapshots`
2. 环境变量：`PAICLI_SNAPSHOT_DIR`
3. 默认值：`~/.paicli/snapshots/<project_hash>/<worktree_hash>/.git`

快照配置读取顺序：

1. 系统属性：`paicli.snapshot.enabled`、`paicli.snapshot.max`、`paicli.snapshot.excludes`、`paicli.snapshot.dir`
2. 环境变量：`PAICLI_SNAPSHOT_ENABLED`、`PAICLI_SNAPSHOT_MAX`、`PAICLI_SNAPSHOT_EXCLUDES`、`PAICLI_SNAPSHOT_DIR`
3. 默认值：启用 / `50` / `.git,.paicli/snapshots,target,node_modules,dist,.idea,*.class,*.jar` / `~/.paicli/snapshots`
...
模型思考过程日志：ReAct / Plan task / SubAgent / Planner 的完整 `reasoning_content` 会以 `LLM reasoning [...]` 写入 `paicli.log`，用于排查模型为什么选择某个工具或路径；请求体里的 base64 图片不会写入日志。
...
设计取舍：长上下文模型（GLM-5.1 200k / DeepSeek V4 1M / StepFun 256k / Kimi K2.6 256k）配合套餐用户的"无限 token"诉求，默认不再以 80% × window 为硬限——让 LLM 自然停在 content 不再调用工具的位置。死循环防护由 stagnation 检测（连续 3 轮相同工具调用）和 hardMaxIterations（50 轮）两道兜底承担。需要严格成本控制的 CI / 自动化批跑场景通过系统属性显式开启硬预算。
...
- 日常小改优先跑针对性 `-Dtest=...`，不要机械全量跑旧测试
- 第 16 期终端 / TUI / inline renderer 相关改动优先跑 `mvn test -Pphase16-smoke`
- 常规回归跑 `mvn test -Pquick`，会跳过外部进程 / 网络超时 / 命令超时类慢测试
- 发版、大范围重构、MCP / web / tool 执行链改动前再跑完整 `mvn test`
...
- GLM-5.1 当前声明 `200000` window；DeepSeek V4 当前声明 `1000000` window；StepFun 当前声明 `256000` window；Kimi K2.6 当前声明 `256000` window
...
- 通过 `/hitl` 查看当前状态
- 危险工具：`write_file`（中危）、`execute_command`（高危）、`create_project`（中危）、`revert_turn`（高危）
...
- `AuditLog`：危险工具（`write_file` / `execute_command` / `create_project` / `revert_turn`）调用一律落一行 JSONL 到 `~/.paicli/audit/audit-YYYY-MM-DD.jsonl`，字段：`timestamp / tool / args / outcome (allow|deny|error) / reason / approver (hitl|policy|none) / durationMs`。审计写入失败仅 stderr 提示，不影响主流程
...
- `image` 类型 content 会作为图片附件附加到下一轮 LLM 请求；`McpCallToolResult` 同时保留 fallback 文案，用于日志、人类可读摘要和 API 不接受图片时的上下文
...
### 14. TUI 界面（v16.1 形态修正后：双形态可切换）

v16.1 引入 `Renderer` 接口（`com.paicli.render.Renderer`），把对话流核心交互（流式输出、工具调用、HITL、状态栏、行内 diff、palette）收口到一个抽象层，三个实现并存：

- **`InlineRenderer`（默认）**：Claude Code 风格的 inline 流式 TUI——不进 alternate screen，主屏直接输出，DECSTBM 滚动区域留出底部常驻状态栏；工具调用 `Read 3 files (ctrl+o to expand)` 行内可折叠；行内 diff（红减绿加 + `@@` hunk）；HITL 单字符 `[y/n/a/s/m]` 提示；`/config` palette 浮起列表。
- **`LanternaRenderer`**：v16 全屏三栏 TUI（保留），通过 `PAICLI_RENDERER=lanterna` 或兼容旧设置 `PAICLI_TUI=true` 启用。所有 widget（CenterPane / StatusPane / FileTreePane / InputBar）保持不变；`LanternaRenderer` 是薄适配器，把 Renderer 方法转成 `runOnGuiThread` widget 调用。
- **`PlainRenderer`**：纯 println 兜底，`PAICLI_RENDERER=plain` 启用，等价 v15 行为。

#### 启动与切换

- 默认（无任何 env）：inline 流式 TUI
- `PAICLI_RENDERER=inline|lanterna|plain` 显式选择
- `PAICLI_TUI=true`（旧）→ 等价 lanterna，打 deprecation 提示
- `PAICLI_NO_STATUSBAR=true` 或 `-Dpaicli.no.statusbar=true`：inline 模式禁用底部状态栏（不支持 DECSTBM 的终端用）
- `NO_COLOR=1`：禁用所有 ANSI 颜色，保留布局

#### 包结构

- `com.paicli.render.Renderer`：核心接口（stream / appendToolCalls / appendDiff / updateStatus / promptApproval / openPalette）
- `com.paicli.render.PlainRenderer`：纯 println
- `com.paicli.render.RendererFactory`：mode 解析 + 创建（不支持的能力自动降级）
- `com.paicli.render.inline.*`：
  - `AnsiSeq`：ANSI 控制序列常量（cursor / clear / scroll region / reverse）
  - `TerminalCapabilities`：终端能力探测
  - `BottomStatusBar`：DECSTBM 底部常驻状态栏（200ms 节流重绘）
  - `FoldableBlock` + `BlockRegistry`：行内可折叠工具块
  - `ToolCallRenderer`：把工具调用转成 FoldableBlock
  - `InlineDiffRenderer`：行内 diff
  - `InlineApprovalPrompter`：HITL 单字符提示
  - `SlashPalette`：行内浮起命令选择列表
- `com.paicli.tui.LanternaRenderer`：薄适配器（保留 v16 全屏 TUI 工作）
- `com.paicli.tui.*`：v16 widget 代码原样保留
- `com.paicli.hitl.RendererHitlHandler`：与 Renderer 协作的 HITL 处理器，状态自管理，UI 委托给 `Renderer.promptApproval`

#### TUI 内支持命令（与 v16 一致）

`/clear`、`/context`、`/memory`、`/memory clear`、`/save <事实>`、`/hitl`、`/hitl on`、`/hitl off`、`/snapshot`、`/snapshot status`、`/snapshot clean`、`/restore <N>`、`/config`、`/plan <任务>`、`/team <任务>`、`/cancel`、`/exit`

inline 模式下命令行渲染走 JLine LineReader（`Ctrl+O` 绑定 → `BlockRegistry.toggleLast()`）；lanterna 模式下走 InputBar widget。

#### Inline 模式的 ANSI 兼容性

DECSTBM 滚动区域在 macOS Terminal、iTerm2、Alacritty、Kitty、Windows Terminal 都受支持；老 Windows cmd / 部分 SSH 客户端不支持时通过 `PAICLI_NO_STATUSBAR=true` 禁用，其它 inline 特性照常。

### 15. LSP 诊断注入（第 17 期 MVP）

第 17 期当前落地的是轻量 MVP：不先引入真实 JDT LS 进程，而是复用已有 JavaParser 依赖，对 `write_file` 成功后的 Java 文件做 post-edit 语法诊断，并把诊断作为下一轮 LLM 请求前的合成 user message 注入。后续接 JDT LS / rust-analyzer / pyright / gopls 时，复用同一条 `LspManager` → `ToolRegistry` → Agent 注入链路。

- 主模块：`src/main/java/com/paicli/lsp/`
- `LspManager`：持有项目根、按文件维护 pending diagnostics；`runPostEditLspHook(path, safePath)` 在文件写入成功后触发；`flushPendingDiagnostics()` 一次性取出并清空 pending 诊断
- `LspDiagnosticFormatter`：按 severity（error / warning / info）、文件路径、行列号、message 格式化；默认最多注入 20 条，可用 `PAICLI_LSP_MAX_DIAGNOSTICS` 或 `-Dpaicli.lsp.max.diagnostics` 覆盖
- `ToolRegistry.write_file`：成功写入并渲染 diff 后调用 `runPostEditLspHook`；诊断失败只打 trace，不影响工具结果
- `Agent` / `PlanExecuteAgent` / `SubAgent`：每轮调用 LLM 前先 `flushPendingLspDiagnostics()`，有诊断时渲染给用户并向消息历史追加 `[LSP 诊断注入]` 合成 user message
- `PAICLI_LSP_ENABLED=false` 或 `-Dpaicli.lsp.enabled=false` 可关闭该能力
- 当前边界：只覆盖 Java 语法级诊断，不做完整 Maven 编译、不启动真实 LSP server、不覆盖 Rust/Python/Go

### 16. Git Side-History 快照与回滚（第 18 期 MVP）

第 18 期当前落地的是 turn 级文件安全网：不污染用户项目 `.git`，而是在 `~/.paicli/snapshots/<project_hash>/<worktree_hash>/.git` 维护独立 side-git 仓库。实现使用 JGit 纯 Java API，不依赖系统 `git` 命令。

- 主模块：`src/main/java/com/paicli/snapshot/`
- `SideGitManager`：初始化 side-git、写入 exclude、创建 `pre-turn` / `post-turn` / `pre-restore` commit、列快照、恢复目标 tree
- `SnapshotService`：ReAct / Plan / Team / TUI 共用的 turn wrapper；pre-turn 同步完成后再进入 Agent，post-turn 进入单线程 executor 异步写入
- `ToolRegistry`：持有共享 `SnapshotService`，注册 `revert_turn` 工具；Plan / Team 继续复用 ReAct 的同一个 `ToolRegistry`，避免快照状态分裂
- CLI 命令：`/snapshot`、`/snapshot status`、`/snapshot clean`、`/restore <N>`
- TUI 命令：同样支持 `/snapshot`、`/snapshot status`、`/snapshot clean`、`/restore <N>`
- `revert_turn` 纳入 HITL / AuditLog 危险工具链；恢复前会先创建 `pre-restore` 快照
- 默认 excludes：`.git`、`.paicli/snapshots`、`target`、`node_modules`、`dist`、`.idea`、`*.class`、`*.jar`
- 当前边界：恢复按 side-git 已跟踪文件写回，不做交互式 merge；`/snapshot clean` 清理当前项目 side-git 目录；`maxSnapshots` 当前用于列表/定位上限，历史压缩留作后续增强

### 17. Prompt 分层架构（第 19 期 MVP）

第 19 期当前落地的是运行模式 system prompt 的资源化：ReAct、Plan task executor、Multi-Agent 三角色、Planner 都不再维护 Java 内硬编码长 prompt，而是通过 `PromptAssembler` 从 Markdown 资源组装。

- 主模块：`src/main/java/com/paicli/prompt/`
- 内置资源：`src/main/resources/prompts/`
...
- 覆盖方式：整文件替换，不做局部 merge
- 必要校验：`base.md` 和最终 prompt 必须包含 `## Language`
- 动态段：长期记忆、MCP resource index、Skill index 由 `PromptContext` 注入到靠后的 volatile section
- 当前边界：Memory 压缩 / 事实抽取 prompt 仍在 `memory` 包内；后续可独立拆到 `prompts/memory/`
- Prompt 改动审计模板：`docs/prompt-analysis-template.md`

### 18. 异步后台任务 + Runtime API（第 20 期 MVP）

第 20 期当前落地的是后台任务队列和 localhost Runtime API 的最小可用闭环。

- 主模块：`src/main/java/com/paicli/runtime/task/`、`src/main/java/com/paicli/runtime/api/`
- `DurableTaskManager`：SQLite 任务队列，默认 `~/.paicli/tasks/tasks.db`
- 任务生命周期：`enqueued` → `running` → `completed` / `failed` / `canceled`
- 启动恢复：上次进程残留的 `running` 会恢复为 `enqueued`
- CLI 命令：`/task`、`/task list [N]`、`/task add <任务内容>`、`/task cancel <task_id>`、`/task log <task_id>`
- Worker 数：默认 2，可用 `PAICLI_TASK_WORKERS` 或 `-Dpaicli.task.workers` 覆盖
- Runtime API 启动：`java -jar target/paicli-1.0-SNAPSHOT.jar serve --http --port 8080`
- Runtime API 仅监听 `127.0.0.1`，强制要求 `PAICLI_RUNTIME_API_KEY` 或 `-Dpaicli.runtime.api.key`
- 端点：`POST /v1/threads`、`POST /v1/threads/{id}/turns`、`GET /v1/threads/{id}/events`
- 当前边界：SSE 是事件回放式，不做长连接持续阻塞推送；后台任务 runner 使用 headless ReAct Agent，不复用交互式 TUI 的 HITL 输入
- 详细文档：`docs/phase-20-runtime-api.md`

### 19. 图片复制粘贴输入（第 21 期 MVP）

第 21 期当前落地的是图片复制粘贴输入主链路，不在公共接口里声明或拦截图像能力。

- `LlmClient.Message` 新增 `ContentPart`，保留旧字符串构造器兼容现有代码
- `AbstractOpenAiCompatibleClient`：纯文本消息仍输出 string content；含图片时输出带图片块的 content array
- 公共 `LlmClient` 接口不做图片能力声明；输入层只负责读取、压缩、附加图片，provider API 负责最终接收或返回错误
- GLM 套餐用户可通过 `/model glm-5v-turbo` 切到 GLM-5V-Turbo 多模态模型，再用 Ctrl+V 或 `@image:` 输入图片；`GLMClient` 会自动改走 `https://open.bigmodel.cn/api/paas/v4/chat/completions`，本地 base64 图片会按智谱格式写入 `image_url.url`
- `McpContent` 读取 MCP image content 的 `data` / `mimeType`；`McpCallToolResult.toToolOutput()` 同时返回文本 fallback 与图片附件
- `ToolRegistry.ToolExecutionResult` 保留 `imageParts`，ReAct / Plan / SubAgent 在工具结果后追加图片 user message
- `ImageProcessor` 统一处理本地图片和 MCP 图片：不做 OCR 文本化，保留为 `base64 + mimeType` image block；带 alpha 的 PNG 会铺白底重编码；大图先等比缩放到 2000x2000 范围内并压缩到 5MB base64 API 上限内；额外注入来源、尺寸和坐标映射元信息
- `ImageReferenceParser` 会把本地 `@image:` 图片 block 放在最后，并在文本提示中要求优先分析本轮图片；除非用户明确要求结合历史，历史对话、历史工具结果、网页 / 仓库信息不能替代当前图片内容
- ReAct / SubAgent 在新一轮任务开始前会把历史 image payload 替换为文本占位，保留 `Image source` 等元信息，避免旧截图反复消耗上下文或污染新图分析；模型 `reasoning_content` 只用于日志 / 流式展示，不应回传进下一轮 LLM 请求历史
- 用户输入支持 `@image:file:///abs/path.png`、`@image:/abs/path.png`、`@image:relative/path.png`，只接受 `image/*`
- 当前边界：不做视频 / 音频、图像生成、TUI sixel 图片预览；图片 token 成本只做粗略估算，真实计费以 provider usage 为准
- 详细文档：`docs/phase-21-image-input.md`
...
│   ├── LspManager.java
│   ├── LspDiagnostic.java
│   ├── LspDiagnosticFormatter.java
│   ├── LspDiagnosticReport.java
│   └── LspSeverity.java
├── prompt/
...
├── image/
│   └── ImageReferenceParser.java
├── runtime/
│   ├── api/
│   │   ├── RuntimeApiServer.java
│   │   └── RuntimeThreadStore.java
│   └── task/
│       ├── DurableTaskManager.java
│       ├── DurableTask.java
│       └── TaskCommandFormatter.java
├── snapshot/
│   ├── SideGitManager.java
...
### `src/main/java/com/paicli/tui/`

- `TuiBootstrap.java`：TUI 启动判断、Lanterna 窗口创建、HITL delegate 切换
- `LanternaWindow.java`：封装 Screen / GUI / Window 生命周期
- `TuiSessionController.java`：把 TUI 输入桥接到 ReAct / Plan / Team 执行路径，并捕获输出写回对话面板
- `pane/`：文件树、对话流、状态栏、输入栏
- `hitl/TuiHitlHandler.java`：Lanterna 模态审批框
- `history/ConversationSnapshot.java`：TUI 对话 JSONL 快照
- `highlight/CodeHighlighter.java`：轻量语法高亮

### `src/main/java/com/paicli/llm/*Client.java`

- `GLMClient` 默认模型：`glm-5.1`；默认接口：`https://open.bigmodel.cn/api/coding/paas/v4/chat/completions`；当模型名以 `glm-5v` 开头（如 `glm-5v-turbo`）时，切到多模态接口 `https://open.bigmodel.cn/api/paas/v4/chat/completions`
- `DeepSeekClient` 默认模型：`deepseek-v4-flash`；接口：`https://api.deepseek.com/chat/completions`
- `StepClient` 默认模型：`step-3.5-flash`；默认 base URL：`https://api.stepfun.com/v1`，最终请求 `.../chat/completions`；可通过 `STEP_BASE_URL` 切到 Step Plan 专用通道 `https://api.stepfun.com/step_plan/v1`
- `KimiClient` 默认模型：`kimi-k2.6`；默认 base URL：`https://api.moonshot.ai/v1`，最终请求 `.../chat/completions`；可通过 `KIMI_BASE_URL` / `MOONSHOT_BASE_URL` 覆盖；Kimi thinking + tool calls 会把上一轮 assistant 的 `reasoning_content` 带回请求历史，满足 K2.6 工具调用协议约束
...
如果修改 `/plan`、`/team`、`/cancel`、`/hitl`、`/mcp`、`/policy`、`/audit`、`/snapshot`、`/restore`、`/clear`、`/memory`、`/save`、`/index`、`/search`、`/graph`、`/exit` 或输入解析：
...
如果修改 GLM / DeepSeek / StepFun / Kimi 模型名、接口地址、认证方式或配置项：
...
- 跑相关 `-Dtest=...` 或 `mvn test -Pquick` 即可，不要求每次全量
...
- API/模型问题：先看 `src/main/java/com/paicli/llm/*Client.java` 和 `LlmClientFactory.java`
