...
- 下一步：第 14 期 `CDP 会话复用`、第 15 期 `Skill 系统`、第 16 期 `TUI 产品化`、第 17 期 `多模态 LLM 输入`；OAuth / sampling / recovery 作为后续 MCP 增强，不属于当前第 13 期已交付范围
- 当前用户可感知版本：CLI Banner 显示 `v13.0.0`
- 当前 Maven 产物版本：`pom.xml` 仍是 `1.0-SNAPSHOT`
- 结论：如果你看到运行界面是 `v13.0.0`，但 Jar 名仍是 `paicli-1.0-SNAPSHOT.jar`，这是当前仓库的真实状态，不是你看错
...
2. 默认值：`80% * 当前模型 maxContextWindow` / `3` / `50`
...
如果已有用户级配置但缺少 `chrome-devtools`，启动只打印提示，不自动修改用户文件。默认 isolated 模式不复用日常 Chrome 登录态；第 14 期再处理 `--browser-url` 会话复用。
...
- `web_fetch` 走「OkHttp + Jsoup + 简易 readability」本地链路，对静态/SSR 页面有效；遇到 SPA / 防爬墙会返回空正文 + `已知边界` 提示，不会反复重试。JS 渲染可 fallback 到第 13 期 Chrome DevTools MCP；登录态访问留给第 14 期 CDP 会话复用
...
- 边界明确：SPA / 防爬墙会返回 `body_empty: true` + `已知边界`提示，调用方不重试。JS 渲染可 fallback 到 Chrome DevTools MCP；登录态留给第 14 期 CDP 会话复用
...
第 13 期交付浏览器能力，但不自研浏览器自动化协议，直接接 Google 官方 `chrome-devtools-mcp@latest`。
...
- 当前边界：默认 isolated 模式不保留 cookie / 登录态；复用已登录 Chrome、敏感页面识别和 `--browser-url` 连接留给第 14 期
...
这意味着当前自动化测试更偏解析、计划结构、RAG 核心模块、Multi-Agent 编排逻辑、HITL 审批策略、策略层拦截规则、MCP 协议核心组件、MCP resources 输入层和长上下文策略计算，不覆盖真实 LLM 联调、真实 Embedding API 联调、真实 npm/uvx MCP server 联调，也不覆盖终端交互的完整手工体验。
