...
如果已有用户级配置但缺少 `chrome-devtools`，启动只打印提示，不自动修改用户文件。默认 isolated 模式不复用登录态；需要登录态时使用 `/browser connect [port]` 在当前进程内切到 `--browser-url=http://127.0.0.1:<port>`，复用带远程调试端口的 Chrome。
...
- `/browser status`：查看当前 isolated / shared 模式、chrome-devtools 状态和 9222 探活结果
- `/browser connect [port]`：默认探活 9222，成功后把 `chrome-devtools` 运行时切到 `--browser-url=http://127.0.0.1:<port>` 并重启该 server；失败不修改 args
