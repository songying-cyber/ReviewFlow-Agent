...
- Java 17+
- Maven
- 可用的 `GLM_API_KEY`、`DEEPSEEK_API_KEY` 或 `STEP_API_KEY`

API Key 当前读取顺序以代码为准：

1. `~/.paicli/config.json` 中对应 provider 的 `apiKey`
2. 环境变量：`GLM_API_KEY` / `DEEPSEEK_API_KEY` / `STEP_API_KEY`
3. 仓库当前目录下的 `.env`
4. 用户主目录下的 `.env`
...
设计取舍：长上下文模型（GLM-5.1 200k / DeepSeek V4 1M / StepFun 256k）配合套餐用户的"无限 token"诉求，默认不再以 80% × window 为硬限——让 LLM 自然停在 content 不再调用工具的位置。死循环防护由 stagnation 检测（连续 3 轮相同工具调用）和 hardMaxIterations（50 轮）两道兜底承担。需要严格成本控制的 CI / 自动化批跑场景通过系统属性显式开启硬预算。
...
- GLM-5.1 当前声明 `200000` window；DeepSeek V4 当前声明 `1000000` window；StepFun 当前声明 `256000` window
...
- `image` 类型 content 当前仍不进入多模态 LLM 输入（已后移到第 21 期 vision），`McpCallToolResult` 只返回 fallback 文案，引导改用 `take_snapshot` 获取 DOM 文本
...
- `GLMClient` 默认模型：`glm-5.1`；接口：`https://open.bigmodel.cn/api/coding/paas/v4/chat/completions`
...
如果修改 GLM / DeepSeek / StepFun 模型名、接口地址、认证方式或配置项：
